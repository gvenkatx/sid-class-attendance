...
def load_known_faces_from_db():
    ...
    table_name = 'class_face_data'
    if table_exists(conn, table_name):
      read_query = 'SELECT face_encoding, face_metadata, face_location FROM class_face_data'
      cursor.execute(read_query)
      rows = cursor.fetchall()
      for row in rows:
        ##print("encoding: ",pickle.loads(row[0]),"\nmetadata: ", pickle.loads(row[1]), "\nlocation: ", pickle.loads(row[2]))
        db_face_encodings.append(pickle.loads(row[0]))
        face_metadata = pickle.loads(row[1])
        face_location = pickle.loads(row[2])
        face_metadata['face_image'] = face_location
        db_face_metadata.append(face_metadata)
    ...
    if db_face_encodings:
      with open("known_faces.dat", "wb") as face_data_file:
        face_data = [db_face_encodings, db_face_metadata]
        pickle.dump(face_data, face_data_file)
        print("Known faces loaded to disk from db.")
    else:
      print("Starting from empty db.")
